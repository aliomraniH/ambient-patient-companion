...
class TieredContextLoader:
    ...
    async def load_tier1(self) -> dict:
        ...
        log.info(f"Tier 1 loaded: {len(serialized)} chars")
        return ctx
    ...
    async def load_tier2(self, requested_tests: list[str] | None = None) -> dict:
        ...
        log.info(f"Tier 2 loaded: {len(serialized)} chars, total: {self._chars_used}")
        return ctx
    ...
    async def load_on_demand(self, data_request: dict) -> dict:
        ...
        if self._chars_used >= TOTAL_BUDGET:
            log.warning(f"on-demand fetch skipped (budget exhausted): {req_type}")
            return {}
        ...
        async with self.db_pool.acquire() as conn:
            ...
            if req_type == "clinical_note":
                # Fetch sanitized note text — NEVER the raw blob
                if resource_id:
                ...
                # Imaging report text from clinical_notes (already sanitized at ingest)
                if resource_id:
        ...
        serialized = json.dumps(ctx)
        self._chars_used += len(serialized)
        log.info(f"On-demand fetch [{req_type}]: {len(serialized)} chars, total: {self._chars_used}")
        return ctx
